...
class ImageMagick {
	 ...
	 */
	public function add_watermark($water,$x = 10, $y = 10, $isfile = true){
		if(!C('IMAGE_WATER_CONFIG.status')){
			return false;
		}
		if(C('IMAGE_WATER_CONFIG.minWidth') && $this->get_width() < C('IMAGE_WATER_CONFIG.minWidth')){
			return false;
		}
		if(C('IMAGE_WATER_CONFIG.minHeight') && $this->get_height() < C('IMAGE_WATER_CONFIG.minHeight')){
			return false;
		}
		//水印位置  1-9九宫格
		$position = preg_match("/^[1-9]$/", C('IMAGE_WATER_CONFIG.position')) ? C('IMAGE_WATER_CONFIG.position') : 0;
